...
const statusStyles: Record<Report["status"], string> = {
  Reviewed: "bg-warning-100 text-warning-600",
  Available: "bg-success-100 text-success-600",
  Pending: "bg-error-100 text-error-600",
};
...
type ReportsTableProps = {
  ...
  onDownload: (row: Report) => void;
  onShare: (row: Report) => void;
  ...
  onSelectionRowsChange?: (rows: Report[]) => void;
};
...
export default function ReportsTable({
  ...
  const columns = useMemo<ColumnDef<Report>[]>(
    () => [
      ...
      { accessorKey: "analysis", header: "AI Report Analysis" },
      { accessorKey: "doctorComment", header: "Doctor's Comment" },
      { accessorKey: "reportResult", header: "Report Result" },
  ...
  const table = useReactTable({
    ...
    // ✅ IMPORTANT: stable row IDs (use your real report id)
